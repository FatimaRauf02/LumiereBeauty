...
import { withRetry, GEMINI_MODEL } from "../lib/openai";
...
router.post("/upload-image", async (req: any, res) => {
  try {
    ...
    if (!imageData || !fileName) {
      res.status(400).json({ message: "imageData and fileName are required" });
      return;
    }
...
router.get("/sales-analytics", async (req, res) => {
  try {
    ...
    const products = await db
      .select({ name: productsTable.name, reviewCount: productsTable.reviewCount })
      .from(productsTable);
    ...
    const topProducts = products
      ...
      .map((p) => ({
        name: p.name,
        sales: p.reviewCount + Math.floor(Math.random() * 50),
      }));
