...
router.get("/products", async (req, res) => {
  try {
    const {
      category, subcategory, skinType, minPrice, maxPrice,
      minRating, inStock, sort, search, page = "1", limit = "12",
    ...
    if (skinType) filtered = filtered.filter(p => (p.skinTypes as string[]).includes(skinType));
    if (minRating) filtered = filtered.filter(p => parseFloat(p.rating) >= parseFloat(minRating));
    if (search) {
...
router.get("/products/featured", async (req, res) => {
  try {
    ...
    const result = products.map(formatProduct);
...
router.get("/products/categories", async (req, res) => {
  try {
    ...
    const result = Array.from(categoryMap.entries()).map(([name, subs]) => ({
      name,
...
router.get("/products/:id/related", async (req, res) => {
  try {
    ...
    const related = await db.select().from(productsTable)
      ...
      .limit(4);
    const result = related.map(formatProduct);
    setCache(cacheKey, result);
    res.json(result);
  } catch (err) {
    req.log.error(err);
