    ...
    <div className="bg-background">
      ...
      <section ref={heroRef} className="relative min-h-screen overflow-hidden flex items-center">
        ...
        <div className="relative w-full max-w-7xl mx-auto px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center py-24 pt-32">
          ...
          <motion.div style={{ y: textY, opacity }} className="flex flex-col gap-7 order-2 lg:order-1">
            ...
            >
              <span className="w-8 h-px bg-primary" /> Premium Skincare & Beauty
            </motion.span>
            ...
            <motion.div
              initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65, duration: 0.7 }}
            ...
            >
              ...
              >
                Build a Bundle <Sparkles size={13} />
              </Link>
            </motion.div>
          ...
          {/* Right – hero image */}
          <motion.div style={{ y: imgY }} className="relative order-1 lg:order-2 flex justify-center lg:justify-end">
            <motion.div
            ...
            >
              ...
              <div className="relative rounded-full overflow-hidden aspect-square shadow-2xl border-4 border-white">
                <img src="/hero-product.jpg" alt="Luxury skincare products" className="w-full h-full object-cover" />
              ...
              >
                <div className="w-11 h-11 rounded-lg overflow-hidden flex-shrink-0">
                  <img src="https://images.unsplash.com/photo-1620916566398-39f1143ab7be?w=80&q=80&fit=crop" alt="Serum" className="w-full h-full object-cover" />
                </div>
                <div>
                  ...
                  <div className="flex mt-0.5">{[1,2,3,4,5].map(s => <span key={s} className="text-primary text-[8px]">★</span>)}</div>
                </div>
              </motion.div>

              <motion.div
                initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.2, duration: 0.7 }}
      ...
      <section className="bg-white border-y border-border py-12 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Skincare", img: "/skincare.jpg", href: "/products?category=Skincare" },
              { label: "Hair Care", img: "/haircare.jpg", href: "/products?category=Hair+Care" },
              { label: "Body Care", img: "/bodycare.jpg", href: "/products?category=Body+Care" },
              { label: "Sets & Bundles", img: "/bundles.jpg", href: "/products?category=Sets+%26+Bundles" },
            ].map((cat, i) => (
              <motion.div key={cat.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                <Link href={cat.href} className="group relative overflow-hidden rounded-lg aspect-[4/5] block shadow-sm hover:shadow-md transition-shadow">
                  <img src={cat.img} alt={cat.label} className="w-full h-full object-cover transition-transform duration-600 group-hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-foreground/55 via-transparent to-transparent" />
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <p className="text-white font-serif text-lg font-light">{cat.label}</p>
                    <p className="text-white/70 text-xs font-sans tracking-widest uppercase mt-0.5 group-hover:text-white transition-colors">Shop Now →</p>
                  </div>
                </Link>
              </motion.div>
            ))}
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-end justify-between mb-12">
          ...
          <Link href="/products" className="hidden md:flex items-center gap-2 text-xs tracking-widest uppercase text-muted-foreground hover:text-primary transition-colors font-sans">
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-end justify-between mb-12">
          ...
          <Link href="/products?sort=bestSelling" className="hidden md:flex items-center gap-2 text-xs tracking-widest uppercase text-muted-foreground hover:text-primary transition-colors font-sans">
            View All <ArrowRight size={13} />
          </Link>
        </motion.div>
      ...
      <section className="bg-white border-y border-border py-16">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {VALUES.map((v, i) => (
              <motion.div key={v.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="flex flex-col items-center gap-3">
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-end justify-between mb-12">
          ...
          <Link href="/products?sort=newArrivals" className="hidden md:flex items-center gap-2 text-xs tracking-widest uppercase text-muted-foreground hover:text-primary transition-colors font-sans">
            View All <ArrowRight size={13} />
          </Link>
        </motion.div>
      ...
      {recentlyViewed.length > 0 && (
        <section className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-end justify-between mb-12">
            ...
            <button onClick={clearRecentlyViewed} className="hidden md:block text-xs tracking-widest uppercase text-muted-foreground hover:text-foreground transition-colors font-sans">
              Clear
            </button>
          </motion.div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {recentlyViewed.slice(0, 8).map((p, i) => (
              <motion.div key={p.id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07, duration: 0.5 }}>
                <ProductCard product={p} />
              </motion.div>
            ))}
          </div>
      ...
      <section className="bg-primary py-16 text-center">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-xl mx-auto px-6">
          <p className="text-primary-foreground/80 text-[11px] tracking-[0.4em] uppercase font-sans mb-3">Free Shipping Over $75</p>
          <h2 className="font-serif text-3xl md:text-4xl font-light text-primary-foreground mb-6">Your Glow Awaits</h2>
          <Link href="/products" className="inline-flex items-center gap-3 bg-white text-primary px-10 py-4 text-xs tracking-widest uppercase hover:bg-primary-foreground/90 transition-all font-sans shadow">
