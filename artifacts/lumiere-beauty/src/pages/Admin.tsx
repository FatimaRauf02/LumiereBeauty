...
import { BarChart3, Package, ShoppingCart, Users, Star, TrendingUp, AlertTriangle, Plus, Pencil, Trash2, X, Check, Loader2, Upload, Info, Layers, ChevronDown, ChevronRight } from "lucide-react";
import {
  ...
  useAdminCreateProduct, useAdminUpdateProduct,
...
const SUBCATEGORIES_BY_CATEGORY: Record<string, string[]> = {
  "Skincare": ["Cleansers","Toners","Serums","Moisturizers","Cream","Lotion","Eye Cream","Face Masks","Scrubs","Exfoliants","SPF","Sunscreen","Night Cream","Face Oil","Lip Care","BB & CC Cream","Retinol","Vitamin C","Niacinamide","Hyaluronic Acid","Primers","Mists"],
  "Hair Care": ["Shampoo","Conditioner","Hair Masks","Hair Oils","Scalp Treatments","Leave-in Conditioner","Dry Shampoo","Hair Serum","Heat Protectant","Hair Spray","Hair Mousse","Hair Color","Deep Conditioner"],
  "Body Care": ["Body Lotions","Body Butter","Body Cream","Body Scrubs","Body Oils","Body Wash","Shower Gel","Hand Cream","Hand Wash","Foot Cream","Bath Salts","Bath Bombs","Deodorant","Cellulite Cream"],
  "Sets & Bundles": ["Sets & Bundles","Gift Sets","Travel Kits","Routine Kits","Duo Sets"],
};
...
interface ProductFormData {
  ...
  price: string; salePrice: string; images: string; stock: string;
...
function ProductModal({ product, onClose, onSaved }: { product?: any; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState<ProductFormData>(() => {
    if (product) {
      return {
        ...
        category: product.category ?? "Skincare", subcategory: product.subcategory ?? "Moisturizers",
        ...
        skinTypes: product.skinTypes ?? [], concerns: product.concerns ?? [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      skinTypes: form.skinTypes, concerns: form.concerns,
      isBestSeller: form.isBestSeller, isNewArrival: form.isNewArrival, isFeatured: form.isFeatured,
    };
    try {
      if (isEdit) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && onClose()}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required value={form.name}
                ...
                className={inputClass} placeholder="e.g. Rose Face Serum" />
            </div>
            <div>
              <div className="flex items-center gap-1.5 mb-1.5">
                <label className="text-[10px] tracking-widest uppercase text-muted-foreground font-sans">Slug</label>
                <span title="URL-friendly identifier. Auto-generated from name." className="cursor-help">
                  <Info size={11} className="text-muted-foreground/60" />
                </span>
              </div>
              <input value={form.slug} onChange={e => set("slug", e.target.value)} className={inputClass} placeholder="auto-generated" />
            </div>
          </div>
          ...
          {/* Description — manual only, no AI */}
          <div>
            <label className={labelClass}>Description</label>
            <textarea value={form.description} onChange={e => set("description", e.target.value)}
              rows={4} className={`${inputClass} resize-none`} placeholder="Write a luxurious product description..." />
          </div>
          ...
          <div>
            ...
            <input value={form.ingredients} onChange={e => set("ingredients", e.target.value)}
              className={inputClass} placeholder="Hyaluronic Acid, Niacinamide, Vitamin C" />
          </div>
          ...
          <div>
            ...
            <textarea value={form.howToUse} onChange={e => set("howToUse", e.target.value)}
              rows={2} className={`${inputClass} resize-none`} placeholder="Apply to clean skin..." />
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" step="0.01" min="0" required value={form.price}
                onChange={e => set("price", e.target.value)} className={inputClass} placeholder="45.00" />
            </div>
            <div>
              <label className={labelClass}>Sale Price ($)</label>
              <input type="number" step="0.01" min="0" value={form.salePrice}
                onChange={e => set("salePrice", e.target.value)} className={inputClass} placeholder="Optional" />
            </div>
            <div>
          ...
          <div>
            ...
            <div className="space-y-2">
              <input value={form.images} onChange={e => set("images", e.target.value)}
                className={inputClass} placeholder="https://images.unsplash.com/... (comma-separated)" />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {SKIN_TYPES_ALL.map(st => (
                <button key={st} type="button" onClick={() => toggleArrayVal("skinTypes", st)}
          ...
          <div>
            ...
            <input value={form.concerns.join(", ")}
              onChange={e => set("concerns", e.target.value.split(",").map(s => s.trim()).filter(Boolean))}
              className={inputClass} placeholder="dryness, acne, dark spots" />
          </div>
          ...
          <div className="flex gap-6">
            {([["isBestSeller","Bestseller"],["isNewArrival","New Arrival"],["isFeatured","Featured"]] as [keyof ProductFormData, string][]).map(([key, label]) => (
              <label key={key} className="flex items-center gap-2 cursor-pointer">
                <div onClick={() => set(key, !(form[key] as boolean))}
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm font-sans">
          ...
          <tbody>
              ...
              <tr key={order.id} className="border-t border-border/50 hover:bg-muted/20 transition-colors">
                ...
                <td className="py-3 px-4">
                  <span className={`text-xs capitalize px-2 py-0.5 rounded-full ${
                    ...
                    order.status === "shipped" ? "bg-blue-100 text-blue-600" : "bg-primary/10 text-primary"
                  }`}>{order.status}</span>
                </td>
            ...
            {(orders ?? []).length === 0 && (
              <tr><td colSpan={5} className="py-12 text-center text-muted-foreground font-sans text-sm">No orders yet</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function ReviewsTab() {
  ...
  return (
    <div className="space-y-3">
        ...
        <div key={r.id} className="bg-white border border-border rounded-xl p-4 text-sm font-sans shadow-sm">
          <div className="flex items-start justify-between mb-2">
            ...
            <div className="flex items-center gap-3">
              ...
              {!r.isApproved && <button onClick={() => approve(r.id)} className="text-xs text-primary hover:opacity-70 font-medium">Approve</button>}
              <button onClick={() => remove(r.id)} className="text-xs text-destructive hover:opacity-70">Delete</button>
            </div>
          </div>
      ...
      {(reviews ?? []).length === 0 && <div className="text-center py-12 text-muted-foreground font-sans text-sm">No reviews yet</div>}
    </div>
  );
}
...
function VariantsTab() {
  ...
  const saveVariants = async () => {
    ...
    try {
      ...
      cancelEdit();
      refetch();
    } catch { toast({ title: "Error saving variants", variant: "destructive" }); }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-5">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products..." className={`${inputClass} w-56`} />
              ...
              <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {variants.length === 0 ? (
                      <span className="text-[10px] text-muted-foreground/50 font-sans italic">No variants</span>
                    ) : variants.map(v => (
                ...
                <button onClick={() => isExpanded ? cancelEdit() : startEdit(p)}
                  className={`flex items-center gap-1.5 px-3 py-2 text-xs tracking-widest uppercase font-sans border rounded-lg transition-all flex-shrink-0 ${
                    isExpanded ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/60 hover:text-primary"
                  }`}>
                ...
                {isExpanded && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="overflow-hidden">
                    <div className="border-t border-border p-4 bg-muted/20 space-y-3">
                      <p className="text-[10px] tracking-widest uppercase text-muted-foreground font-sans">Edit Variants</p>
                      <div className="space-y-2">
                        {editingVariants.map((v, i) => (
                          <div key={i} className="flex items-center gap-2">
                            <input value={v.label} onChange={e => updateVariant(i, "label", e.target.value)}
                              placeholder="Label (e.g. 30ml)" className={`${inputClass} flex-1`} />
                            <div className="relative w-32">
                              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground pointer-events-none">$</span>
                              <input type="number" step="0.01" min="0" value={v.price}
                                onChange={e => updateVariant(i, "price", e.target.value)}
                                placeholder="0.00" className={`${inputClass} w-full pl-6`} />
                            </div>
                            <button onClick={() => removeVariant(i)} className="p-2 text-muted-foreground hover:text-destructive transition-colors rounded-lg hover:bg-destructive/10 flex-shrink-0">
                              <Trash2 size={13} />
                            </button>
                          </div>
                        ))}
                        {editingVariants.length === 0 && <p className="text-xs text-muted-foreground font-sans italic py-1">No variants yet.</p>}
                      </div>
                      <div className="flex items-center gap-3 pt-1">
                        <button onClick={addVariant} className="flex items-center gap-1.5 text-xs font-sans px-3 py-2 border border-dashed border-primary/40 text-primary/70 rounded-lg hover:bg-primary/5 hover:border-primary transition-all">
                          <Plus size={12} /> Add Variant
                        </button>
                        <div className="flex-1" />
                        <button onClick={cancelEdit} className="text-xs font-sans px-4 py-2 border border-border rounded-lg hover:bg-muted transition-colors">Cancel</button>
...
function AnalyticsTab() {
  ...
  const weeklyMock = [
    { day: "Mon", revenue: 1240 }, { day: "Tue", revenue: 980 },
    { day: "Wed", revenue: 1560 }, { day: "Thu", revenue: 2100 },
    { day: "Fri", revenue: 1870 }, { day: "Sat", revenue: 2840 }, { day: "Sun", revenue: 1920 },
  ];

  return (
    <div className="space-y-8">
      <div className="bg-white border border-border rounded-xl p-5 shadow-sm">
        <h3 className="text-xs tracking-widest uppercase font-sans mb-5">Weekly Sales</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={weeklyMock} barSize={28}>
              ...
              <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", fontFamily: "Jost", fontSize: 12, borderRadius: 8 }} formatter={(v: any) => [`$${Number(v).toFixed(0)}`, "Revenue"]} cursor={{ fill: "hsl(var(--muted))" }} />
            ...
            return (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} barSize={18}>
                  ...
                  <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", fontFamily: "Jost", fontSize: 12, borderRadius: 8 }} formatter={(v: any) => [`$${Number(v).toFixed(0)}`, "Revenue"]} />
