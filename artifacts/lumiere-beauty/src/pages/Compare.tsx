...
export default function Compare() {
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-6 px-6 text-center pt-28">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-8 pt-28 pb-32">
      {/* Header */}
      <motion.div
      ...
      {items.length === 1 && (
        <div className="mb-8 bg-primary/5 border border-primary/20 rounded-xl px-6 py-4 flex items-center justify-between">
          <p className="text-sm font-sans text-muted-foreground">
            Add <span className="text-foreground font-medium">at least one more product</span> to start comparing.
          </p>
          <Link
            href="/products"
            className="text-xs tracking-widest uppercase text-primary hover:opacity-80 font-sans"
          >
            Browse →
          </Link>
        </div>
      ...
      >
        <table className="w-full border-collapse min-w-[600px]">
          ...
          <tbody>
                  ...
                  <td key={p.id} className="px-6 py-8 align-top border-b border-r border-border last:border-r-0">
                    <div className="relative">
                      <button
                        ...
                        className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-muted flex items-center justify-center hover:bg-destructive/20 hover:text-destructive transition-colors z-10"
                      >
                        <X size={11} />
                      </button>
                      ...
                      <div className="flex items-center gap-2 mb-4 font-sans">
                        ...
                        {displayPrice === minPrice && items.length > 1 && (
                          <span className="text-[9px] tracking-widest uppercase bg-primary/10 text-primary px-1.5 py-0.5 font-sans">
                            Best Value
                          </span>
                        )}
                      </div>
                ...
                return (
                  <Cell key={p.id} highlight={isTop}>
                    <div className="flex items-center gap-2">
                      ...
                      {isTop && (
                        <span className="text-[9px] tracking-widest uppercase text-primary font-sans">Top Rated</span>
                      )}
                    </div>
                  </Cell>
                );
            ...
            {/* Skin Types */}
            <tr>
              <LabelCell>Skin Types</LabelCell>
              {items.map(p => (
                <Cell key={p.id}>
                  {(p.skinTypes as string[])?.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {(p.skinTypes as string[]).map(st => (
                        <span key={st} className="text-[10px] capitalize bg-secondary px-2 py-0.5 font-sans">{st}</span>
                      ))}
                    </div>
                  ) : (
            ...
            {/* Stock */}
            <tr>
              <LabelCell>Availability</LabelCell>
              {items.map(p => (
                <Cell key={p.id}>
                  {p.stock > 0 ? (
                    <span className="text-green-500 text-xs font-sans flex items-center gap-1">
                      <Check size={11} /> In Stock ({p.stock})
                    </span>
                  ) : (
                    <span className="text-destructive text-xs font-sans">Out of Stock</span>
                  )}
                </Cell>
              ))}
            </tr>
